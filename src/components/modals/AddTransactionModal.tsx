...
export const AddTransactionModal = ({ onClose }: AddTransactionModalProps) => {
    ...
    const [formType, setFormType] = useState<'debt' | 'income'>('debt');
    ...
    const totalValue = useMemo(() => {
        ...
        if (debtType === 'installment' && !isNaN(numAmount) && !isNaN(numInstallments) && numAmount > 0 && numInstallments > 0) {
    ...
    const handleSubmit = () => {
        const numAmount = parseAmount(amount);
        if (!name || numAmount <= 0 || (formType === 'debt' && !category)) return toast.error("Preencha Nome, Valor e Categoria corretamente.");
                ...
                <CardContent className="p-4 max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div><Label>{formType === 'debt' ? 'Valor da Parcela' : 'Valor'}</Label><Input type="text" inputMode="decimal" value={amount} onChange={handleAmountChange} placeholder="150,00"/></div>
                        </div>
